...
if ($solution.Extension -eq '.slnx') {
    ...
    $solutionFolderNames = @([regex]::Matches($text,
        '(?m)^Project\("\{2150E333-8FDC-42A3-9474-1A3956D46DE8\}"\) = "([^"]+)"') |
        ForEach-Object { $_.Groups[1].Value })
    $duplicateFolders = @($solutionFolderNames | Group-Object { $_.ToLowerInvariant() } | Where-Object Count -gt 1)
    if ($duplicateFolders) { throw "Duplicate solution folders: $($duplicateFolders.Name -join ', ')" }
